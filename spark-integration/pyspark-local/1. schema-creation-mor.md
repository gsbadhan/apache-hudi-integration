...
BASE_PATH = "/tmp/hudi_employees_mor"
```
...
initial_data = [
    ...
    (4, "Alice Brown", 103, 70000.0, 90000.0, 6.5, datetime(2024, 1, 1, 10, 0, 0)),
    (5, "Charlie Wilson", 102, 72000.0, 160000.0, 8.9, datetime(2024, 1, 1, 10, 0, 0))
]
...
new_employee = spark.createDataFrame(
    [(6, "sam", 103, 70000.0, 90000.0, 6.5, datetime.now())],
    EMPLOYEE_SCHEMA
)
...
empty_df = spark.createDataFrame([], EMPLOYEE_SCHEMA)

empty_df.write.format("hudi").options(**HUDI_OPTIONS).mode("overwrite").save(BASE_PATH)
